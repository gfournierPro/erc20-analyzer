...
func main() {
	...
	if err := messaging.EnsureTopics(bootstrapCtx, cfg.Kafka.Brokers, []messaging.TopicSpec{
		...
		{Name: cfg.Kafka.Topics.SnapshotStatus, Partitions: 6, ReplicationFactr: 1},
	}); err != nil {
	...
	resultsPub := messaging.NewPublisher(cfg.Kafka.Brokers, cfg.Kafka.Topics.SnapshotResults)
	statusPub := messaging.NewPublisher(cfg.Kafka.Brokers, cfg.Kafka.Topics.SnapshotStatus)
	defer resultsPub.Close()
	defer statusPub.Close()
	emitter := snapshot.NewKafkaEmitter(resultsPub, statusPub)

	scanner := snapshot.NewScanner(snapshot.DefaultScannerConfig(), emitter)
	runner := snapshot.NewRunner(reg, scanner)

	consumer := messaging.NewConsumer(
		cfg.Kafka.Brokers,
		cfg.Kafka.Topics.SnapshotJobs,
		"snapshot-service",
	)
	defer consumer.Close()

	log.Info().Msg("snapshot-service redy; consumer jobs")
